...
class IDEIntegrationServer {
      ...
      tools: [
        {
          ...
          description:
            "Get TypeScript/ESLint diagnostics for a file or entire project",
          inputSchema: {
            type: "object",
            properties: {
              uri: {
                type: "string",
                description:
                  "File URI to get diagnostics for (optional, omit for all files)",
              },
            },
          },
        },
  ...
  async getDiagnostics(args) {
    ...
    try {
      ...
      try {
        ...
        execSync(`npx eslint ${targetPath} --format json`, {
          encoding: "utf8",
          stdio: "pipe",
        });
      } catch (error) {
      ...
      const summary = {
        totalErrors:
          diagnostics.typescript.errors +
          diagnostics.eslint.filter((f) => f.errorCount > 0).length,
        totalWarnings:
          diagnostics.typescript.warnings +
          diagnostics.eslint.filter((f) => f.warningCount > 0).length,
  ...
  async formatCode(args) {
    ...
    try {
      ...
      // Run Prettier
      const formatted = execSync(`npx prettier --write ${tempFile}`, {
        encoding: "utf8",
      });
